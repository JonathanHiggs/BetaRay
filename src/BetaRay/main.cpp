...
Scene RandomScene(Image const & image)
{
    auto hittables = std::make_unique<HittableList>();
    ...
    hittables->Objects.emplace_back(std::make_shared<Sphere>(Point(0, -1000, 0), 1000, groundMaterial));
        ...
        for (auto b = -size; b < size; ++b)
        {
            ...
            if (glm::length(center - Point(4, 0.2, 0)) > 0.9)
            {
                if (matRand < 0.6)
                {
                    ...
                    hittables->Objects.emplace_back(std::make_shared<MovingSphere>(
                ...
                else if (matRand < 0.85)
                {
                    ...
                    hittables->Objects.emplace_back(std::make_shared<Sphere>(center, 0.2, material));
                }
                else
                {
                    // glass
    ...
    hittables->Objects.emplace_back(std::make_shared<Sphere>(Point(0, 1, 0), 1.0, material1));

    auto material2 = std::make_shared<Lambertian>(Color(0.4, 0.2, 0.1));
    hittables->Objects.emplace_back(std::make_shared<Sphere>(Point(-4, 1, 0), 1.0, material2));

    auto material3 = std::make_shared<Metal>(Color(0.7, 0.6, 0.5), 0.0);
    hittables->Objects.emplace_back(std::make_shared<Sphere>(Point(4, 1, 0), 1.0, material3));
    ...
    return Scene{ std::move(hittables), camera };
}


Scene TwoPerlinSpheres(Image const & image)
{
    auto hittables = std::make_unique<HittableList>();
    ...
    hittables->Objects.emplace_back(
...
Scene EarthScene(Image const & image)
{
    auto hittables = std::make_unique<HittableList>();
    ...
    auto earthImage = std::shared_ptr<Image>(std::move(Image::LoadFromFile("earthmap.jpg")));
    auto earthTexture = std::make_shared<ImageTexture>(std::move(earthImage));
    ...
    hittables->Objects.emplace_back(
        std::make_shared<Sphere>(
            Point(0, 2, 0), 2, std::make_shared<Lambertian>(earthTexture)));

    auto from           = Point(13.0, 3.0, 3.0);
    auto to             = Point( 0.0, 1.0, 0.0);
    auto up             = Point( 0.0, 1.0, 0.0);
    auto vfov           = 20.0;
    auto aperture       = 0.01;
    auto focalDistance  = glm::length(from - to);
...
int main()
{
    ...
    // Scene
    //auto scene = RandomScene(image);
    auto scene = EarthScene(image);
    ...
    for (auto y = 0u; y < image.Height; ++y)
    {
        for (auto x = 0u; x < image.Width; ++x)
        {
            ...
            for (auto i = 0u; i < samplesPerPixel; ++i)
            {
                ...
                auto ray = scene.Camera.RayFromViewport(u, v);

                color += RayColor(ray, *scene.Hittables);
            }
